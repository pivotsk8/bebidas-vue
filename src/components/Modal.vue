...
const formatearIngredientes = () => {
  ...
  for (let i = 1; i <= 15; i++) {
    if (receta.value[`strIngredient${i}`]) {
      const ingrediente = receta.value[`strIngredient${i}`];
      const cantidad = receta.value[`strMeasure${i}`];

      const ingredienteCatidad = document.createElement('p');
      ingredienteCatidad.classList.add('text-lg', 'text-gray-500');
      ingredienteCatidad.textContent = `${ingrediente}: ${cantidad}`;

      ingredientesDiv.appendChild(ingredienteCatidad);
    }
  }
...
<template>
  <TransitionRoot as="template" :show="modal">
    <Dialog
      as="div"
      class="relative z-10"
      @close="modalStore.handlerClickModal()">
      <TransitionChild
        as="template"
